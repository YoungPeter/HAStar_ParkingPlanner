...
namespace math
{
  ...
  double DistanceTo(const Box2d &bounding_box, const Vec2d &point)
  {
    ...
    const double dx =
        std::abs(x0 * bounding_box.cos_heading_ + y0 * bounding_box.sin_heading_) - bounding_box.half_length_;
    const double dy =
        std::abs(x0 * bounding_box.sin_heading_ - y0 * bounding_box.cos_heading_) - bounding_box.half_width_;
    if (dx <= 0.0)
    {
  ...
  double DistanceTo(const Box2d &bounding_box, const LineSegment2d &line_segment)
  {
    ...
    double x1 = ref_x1 * bounding_box.cos_heading_ + ref_y1 * bounding_box.sin_heading_;
    double y1 = ref_x1 * bounding_box.sin_heading_ - ref_y1 * bounding_box.cos_heading_;
    ...
    double x2 = ref_x2 * bounding_box.cos_heading_ + ref_y2 * bounding_box.sin_heading_;
    double y2 = ref_x2 * bounding_box.sin_heading_ - ref_y2 * bounding_box.cos_heading_;
  ...
  bool IsPointIn(const Box2d &bounding_box, const Vec2d &point)
  {
    const double x0 = point.x_ - bounding_box.center_.x_;
    const double y0 = point.y_ - bounding_box.center_.y_;
    const double dx = std::abs(x0 * bounding_box.cos_heading_ + y0 * bounding_box.sin_heading_);
    const double dy = std::abs(-x0 * bounding_box.sin_heading_ + y0 * bounding_box.cos_heading_);
    return dx <= bounding_box.half_length_ + 1e-10 && dy <= bounding_box.half_width_ + 1e-10;
  }
  ...
  bool HasOverlap(const Box2d &bounding_box, const LineSegment2d &line_segment)
  {
    ...
    corners.emplace_back(Vec2d{bounding_box.center_.x_ + dx1 + dx2, bounding_box.center_.y_ + dy1 + dy2});
    corners.emplace_back(Vec2d{bounding_box.center_.x_ + dx1 - dx2, bounding_box.center_.y_ + dy1 - dy2});
    corners.emplace_back(Vec2d{bounding_box.center_.x_ - dx1 - dx2, bounding_box.center_.y_ - dy1 - dy2});
    corners.emplace_back(Vec2d{bounding_box.center_.x_ - dx1 + dx2, bounding_box.center_.y_ - dy1 + dy2});
